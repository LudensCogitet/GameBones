#include "gbTextureFilePaths.h"

const char *gbTextureFilePaths[GB_TEXTURE_NAME_NUM_NAMES] = {
    "./assets/overlay.png",         // GB_TEXTURE_NAME_OVERLAY
    "./assets/red_guy.png",         // GB_TEXTURE_NAME_GUY
    "./assets/move_room_panel.png", // GB_TEXTURE_NAME_MOVE_ROOM_PANEL
    "./assets/entity_palette.png",  // GB_TEXTIRE_NAME_ENTITY_PALETTE
    "./assets/cable_palette.png",   // GB_TEXTURE_NAME_POWER_GRID_PALETTE
    "./assets/cables.png",          // GB_TEXTURE_NAME_CABELS
    "./assets/switch.png"           // GB_TEXTURE_NAME_SWITCH
};
